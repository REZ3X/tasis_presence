...
import { isWithinSchoolArea, SCHOOL_AREA } from '@/lib/geofence';
...
export default function HomePage() {
    ...
    const geofenceStatus = location
        ? isWithinSchoolArea(location.latitude, location.longitude)
        : null;
                    ...
                    <div className="space-y-8 lg:space-y-6">
                        ...
                        <div className="rounded-2xl px-5 py-6 shadow-2xl"
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                    ...
                                    <div>
                                        {location ? (
                                            <div className="space-y-2">
                                                ...
                                                {geofenceStatus && (
                                                    <div className="p-3 rounded-lg flex items-start gap-2 text-sm"
                                                        style={geofenceStatus.allowed
                                                            ? { background: 'rgba(34, 197, 94, 0.1)', border: '1px solid rgba(34, 197, 94, 0.3)' }
                                                            : { background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)' }
                                                        }>
                                                        {geofenceStatus.allowed
                                                            ? <FaCheckCircle style={{ color: '#22c55e' }} className="mt-0.5" />
                                                            : <FaExclamationCircle style={{ color: '#ef4444' }} className="mt-0.5" />
                                                        }
                                                        <div>
                                                            <p style={{ color: geofenceStatus.allowed ? '#22c55e' : '#ef4444' }} className="font-semibold">
                                                                {geofenceStatus.allowed ? 'Dalam area sekolah' : 'Di luar area sekolah'}
                                                            </p>
                                                            <p className="text-xs text-gray-400 mt-1">
                                                                {geofenceStatus.distance}m dari {SCHOOL_AREA.name} (maks {geofenceStatus.maxDistance}m)
                                                            </p>
                                                            {!geofenceStatus.allowed && (
                                ...
                                <button
                                    type="submit"
                                    disabled={submitting || !capturedImage || picketType === 'choose' || (user?.role !== 'dev' && geofenceStatus && !geofenceStatus.allowed)}
